  ...
  return (
    <>
          ...
          <div className='gap-3 row p-4 d-flex justify-content-evenly align-items-start h-100 overflow-y-auto ' style={{maxHeight:'465px'}}>
            ...
            {availdoc.map(item=>(
               <div onClick={()=>setDoctor({name:item.name,id:item._id.toString()})} className={doctor.id==item._id.toString()?"border border-2 border-info col-5 rounded-3":"col-5 rounded-3"} style={{backgroundColor:'rgb(31, 33, 33)'}}>
